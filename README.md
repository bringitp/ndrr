...
# 運用方針が未定のため現在放棄中(2023 12/7)。運用に興味ある人があれば連絡ください。
...
| `画面のコンセプトなど` | ...?               | 特になし               |
| `基準カラー３種について`| ...?                    | 未着手               |
| `APIがエラーをだしたときの画面など`| ...?                    | 完               |
| `ベースになるCSSの選定など`| ...?                    | MUIに丸投げ               |
...
| `root_endpoints.py`              | ルートエンドポイント                  | 完了               |
| `user_post_endpoints.py`         | ユーザ名変更、新規登録、退会                | 途中               |
...
| 機能名等                      | 残課題概要                                | 進捗状況           |
|---------------------------------|-------------------------------------|--------------------|
| `ドメインの選定`| 暫定的に ron-the-rocker.net                    | 途中               |
| `技術的なトラブル対応`         | ・・・どうしよ                      | 途中               |
| `人間的なトラブル対応`         | 不明。線引きをきっちりしないと巻き込まれがありうる                      | 途中               |


### 進捗状況の説明

- 未着手: タスクがまだ開始されていない状態です。
- 途中: タスクが進行中で、一部完了している状態です。
- 完了: タスクが完了している状態です。
...
なおユーザ認証はKeycloak 22を用いたOAuth 2.0を用いることとする。  
